...
- 개발 단계에서는 **마일스톤 완료 시점마다 `0.x.0` 버전을 CHANGELOG·package.json에 기록**한다(아직 프로덕션 미배포).
- **git 태그는 `1.0.0`부터만 생성**한다 — 0.x는 배포·롤백에 태그를 쓰지 않으므로 CHANGELOG 기록으로 충분 (기존 0.x 태그는 2026-07-13 정리).
...
M7 메모 (Notes) — 짧은 기록(단상·인용·코드·이미지)을 남기는 메모 영역을 추가했다.

### Added

- 메모 목록 페이지 (`/notes`) — 카드 그리드(masonry), 고정(pinned) 메모 그룹 상단 노출, 자유 형식 렌더(한 줄·인용·코드·이미지), 빈 상태, 전역 내비 '메모' 활성화 (#115)
- 메모 태그 필터 · 날짜 연도 표기 (#116)

### Deferred

- 메모 검색(#117)은 규모상 보류 — M7은 코어(/notes) + 태그 필터까지로 종료
