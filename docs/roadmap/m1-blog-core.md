...
- [ ] Navbar / Footer 레이아웃 (데스크톱 텍스트 메뉴 + 모바일 햄버거 트레이)
- [ ] 반응형 레이아웃
- [ ] 다크/라이트 모드 (`next-themes` 설치 + `layout.tsx` ThemeProvider 적용)
- [ ] 홈 페이지 (`/`) — Hero + 최근 글 섹션 + 아카이브 연도별 사이드바
- [ ] 소개 페이지 (`/about`) — 프로필·연락 채널 (정적)
- [ ] MDX 파싱 및 렌더링
- [ ] Draft 모드 (dev 환경 전용)
- [ ] 글 목록 페이지 (`/writing`)
- [ ] 태그 필터 (목록 페이지 내)
- [ ] 글 상세 페이지 (`/writing/[slug]`)
- [ ] 코드 신택스 하이라이팅
- [ ] 코드 블록 복사 버튼
- [ ] 빈 상태 / 로딩 스켈레톤 / 에러 상태 UI (목록·상세)
- [ ] 404 페이지
