...
> 메모는 **제목이 없다** — 짧은 기록이라 본문이 곧 내용이다. 본문은 MDX이며 한 줄 메모·인용·코드 스니펫·이미지 등 자유 형식(체크리스트·to-do 제외)이다. 검색·필터는 현재 보류. 도입 시 본문 제외(ADR-004), 태그를 대상으로 한다.

### 메모 범위 정책

메모는 **짧은 단상·아이디어·인용·코드 스니펫·이미지**다. 다음은 메모가 아니다:

- **학습 노트·시험 정리** — 길이·성격이 글(writing)에 가깝다. `src/content/writing/`에 글로 작성한다.
- **to-do·체크리스트** — 메모는 할 일 관리가 아니다.
...
| 2026-07-13 | 메모 범위 정책 명시 — 학습 노트·to-do는 메모가 아님 | M7 스코프 확정 |
| 2026-07-13 | Note 검색 보류 반영 — 검색·필터 문구 소프트닝 | M7 |
