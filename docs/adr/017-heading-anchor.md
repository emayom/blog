...
- 위 정합성을 보장하기 위해 `github-slugger`를 명시적 직접 의존성(`dependencies`)으로 추가했다. `rehype-slug`의 전이 의존성이지만, TOC 추출 유틸이 직접 import하므로 버전 변경에도 안전하도록 명시한다. 이는 본 결정에 종속된 후속 조치라 별도 ADR로 분리하지 않는다
