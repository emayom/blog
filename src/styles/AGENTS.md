...
7. **색은 테마 인식 토큰으로 처리한다 — 컴포넌트에 `dark:`를 붙이지 않는다.**
   ...
   **`dark:`는 테마마다 디자인이 다를 때만 쓴다.**

   같은 것의 밝기만 뒤집히면(라이트 어두운 글자 ↔ 다크 밝은 글자) 토큰이 흡수한다. 하지만 라이트와 다크가 **아예 다른 색·다른 표현**이면 토큰 하나로 합칠 수 없다 — 이때만 `dark:`를 쓴다.

   ```
   ❌ 밝기만 반전   text-fg dark:text-body-on-dark        본문 글자 (어둡게 ↔ 밝게) → 토큰
   ✅ 다른 디자인   bg-surface-tile-1 dark:bg-primary/85   장식 타일 (회색 → 파랑) → dark:
   ```

   판단: 다크 값이 **라이트와 같은 역할인데 밝기만 다르면** 토큰, **아예 다른 색·의도면** `dark:`. 앞의 경우를 `dark:`로 두면 규칙이 무너진다.
