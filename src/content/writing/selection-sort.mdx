...
draft: false
description: "선택 정렬의 동작 원리와 구현, 버블 정렬과의 차이를 정리합니다."
thumbnail: ""
series: "정렬 알고리즘"
seriesOrder: 2
---
...
## 결론
선택 정렬 또한 비교적 단순한 알고리즘이지만, 버블 정렬과 마찬가지로 시간 복잡도가 `Best`의 경우 O(n²)으로 **입력 값이 커짐에 따라 연산은 제곱수의 비율로 증가**하는 비효율적인 알고리즘이다.

버블 정렬의 경우 인접한 요소들 간의 교환(Swap)이 반복되는 반면, 선택 정렬의 경우 교환 횟수를 최소화할 수 있다.
하지만, 여전히 시간 복잡도가 O(n²)으로 인해 대용량 데이터에서는 효율적이지 않은 알고리즘이다.
