...
draft: false
description: "분할 정복 기반 퀵 정렬의 동작 원리와 파티셔닝, 구현을 정리합니다."
thumbnail: ""
series: "정렬 알고리즘"
seriesOrder: 4
---
...
퀵 정렬의 시간 복잡도는 `Best`, `Average`의 경우 O(n log n), `Worst`의 경우 O(n²)의 시간 복잡도를 가진다.
...
하지만, 작은 데이터셋에서는 다른 간단한 정렬 알고리즘들이 더 효과적일 수 있으며 `Worst` 케이스의 경우 O(n²)의 성능을 나타낼 수 있다.
