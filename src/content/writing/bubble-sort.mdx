...
draft: false
description: "버블 정렬의 동작 원리와 구현, 그리고 최적화 방법을 정리합니다."
thumbnail: ""
series: "정렬 알고리즘"
seriesOrder: 1
---
...
| :---      |   :----:  |   :----:   |   :----:   |   :----:   |   :----:   |   :----:   |
| 버블 정렬(Bubble Sort) | O(n²) | O(n²) | O(n²) | O(1) | ✅ | ✅ |
...
버블 정렬의 시간 복잡도는 `Best`의 경우 O(n)이고, `Worst`, `Average`의 경우 O(n²)이다.
...
시간 복잡도가 O(n²)으로 **입력 값이 커짐에 따라 연산은 제곱수의 비율로 증가**하는 비효율적인 알고리즘이다.
