...
draft: false
description: "최단 경로 문제의 종류와 다익스트라 알고리즘(순차 탐색·우선순위 큐) 구현을 정리합니다."
thumbnail: ""
---
...
> 아래 예시는 별도의 우선순위 큐(Priority Queue) 구현을 사용한다.
...
- 우선순위 큐가 비어 있지 않은 동안 반복한다.
    ...
    - 꺼낸 노드의 현재 거리가 `distance[current]`보다 크다면, 이미 방문되었으므로 건너뛴다.
    - `j`를 0부터 `n`까지 1씩 증가시키며 반복한다.
        - `current`를 경유하여 `j`에 도달하는 거리를 계산한다.
